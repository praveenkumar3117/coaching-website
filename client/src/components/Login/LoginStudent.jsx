...
const LoginStudent = () => {
  ...
  const handleStudentLogin = async ()=>{
    ...
    let result = await fetch("http://localhost:5000/api/User/login/user",
      {
        method:'post',
        headers: {
    ...
    if(result.success){
      ...
      localStorage.setItem("data", JSON.stringify({result, ...{userloggedin: 1, loggedin:true}})); // save data in localstorage
    }else{
            ...
            <div className="xl:ml-20 xl:w-5/12 lg:w-5/12 md:w-8/12 mb-12 md:mb-0">
              <form>
                ...
                <div className="text-center lg:text-left">
                  <button onClick={handleStudentLogin} type="button" className="inline-block px-7 py-3 bg-blue-600 text-white font-medium text-sm leading-snug uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">
                    Login
                  </button>
                </div>
              </form>
